 ...
 */
function createMatchPathAsync(absoluteBaseUrl, paths) {
    var absolutePaths = MappingEntry.getAbsoluteMappingEntries(absoluteBaseUrl, paths);
 ...
 * See the sync version for docs.
 */
function matchFromAbsolutePathsAsync(absolutePathMappings, requestedModule, readJson, fileExists, extensions, callback) {
...
function findFirstExistingPath(tryPaths, readJson, fileExists, doneCallback, index) {
    ...
    if (tryPath.type === "file" ||
        ...
        fileExists(tryPath.path, function (err, exists) {
            ...
            return findFirstExistingPath(tryPaths, readJson, fileExists, doneCallback, index + 1);
        });
    }
        ...
        readJson(tryPath.path, function (err, packageJson) {
            ...
            if (packageJson && packageJson.main) {
                var file_1 = path.join(path.dirname(tryPath.path), packageJson.main);
